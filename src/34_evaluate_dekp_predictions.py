#!/usr/bin/env python3
"""Evaluate DEKP-public-retrained predictions against benchmark truth."""
...
DEFAULT_PREDICTIONS = BASE / "data" / "final" / "dekp" / "dekp_public_retrained_kcat_input_output.csv"
DEFAULT_METADATA = BASE / "data" / "final" / "dekp" / "dekp_kcat_input_metadata.csv"
DEFAULT_OUT_ROWS = BASE / "data" / "final" / "dekp" / "dekp_public_retrained_kcat_predictions_evaluated.csv"
DEFAULT_OUT_METRICS = BASE / "reports" / "tables" / "dekp_public_retrained_eval_metrics.csv"
DEFAULT_OUT_MISSING = BASE / "data" / "final" / "dekp" / "dekp_public_retrained_invalid_or_unpredicted_rows.csv"
DEFAULT_OUT_MISSING_SUMMARY = BASE / "reports" / "tables" / "dekp_public_retrained_invalid_or_unpredicted_summary.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Score DEKP-public-retrained output against benchmark truth.")
...
def detect_prediction(pred: pd.DataFrame, explicit: str = "") -> tuple[pd.Series, str]:
    ...
    raise ValueError(f"No recognized DEKP prediction column found. Columns: {list(pred.columns)}")
...
def combine_predictions(pred: pd.DataFrame, meta: pd.DataFrame, pred_log10: pd.Series, pred_col: str) -> pd.DataFrame:
    ...
    if "dekp_row_id" in pred.columns and "dekp_row_id" in meta.columns:
        pred["dekp_row_id"] = pd.to_numeric(pred["dekp_row_id"], errors="coerce").astype("Int64")
        meta["dekp_row_id"] = pd.to_numeric(meta["dekp_row_id"], errors="coerce").astype("Int64")
        keep = ["dekp_row_id", "prediction_log10", "prediction_column"] + optional
        combined = meta.merge(pred[keep], on="dekp_row_id", how="inner", validate="one_to_one")
        ...
        raise ValueError("Need dekp_row_id or entry_id in both predictions and metadata.")

    combined["true_kcat_log10"] = numeric(combined["true_kcat_log10"])
    combined["true_kcat"] = numeric(combined["true_kcat"])
    if "prediction_kcat" not in combined.columns:
...
def missing_rows(meta: pd.DataFrame, rows: pd.DataFrame) -> pd.DataFrame:
    if "dekp_row_id" in meta.columns and "dekp_row_id" in rows.columns:
        return meta.loc[~meta["dekp_row_id"].isin(rows["dekp_row_id"])].copy()
...
def build_metrics(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    if "dekp_train_exact_pair_overlap" in rows.columns:
        for overlap, part in rows.groupby("dekp_train_exact_pair_overlap", sort=True):
            metrics.append(score_group("train_exact_pair_overlap", str(bool(overlap)), part))
    return pd.DataFrame(metrics)


def missing_reason(row: pd.Series) -> str:
    if not bool(row.get("dekp_smiles_valid", True)):
        return "invalid_smiles"
    if not bool(row.get("dekp_structure_available", True)):
        return "missing_structure"
    return "failed_graph_or_unpredicted"
