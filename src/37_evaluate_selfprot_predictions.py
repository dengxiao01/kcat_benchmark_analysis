#!/usr/bin/env python3
"""Evaluate SELFprot predictions against the finalized benchmark truth."""
...
DEFAULT_PREDICTIONS = BASE / "data" / "final" / "selfprot" / "selfprot_kcat_input_output.csv"
DEFAULT_METADATA = BASE / "data" / "final" / "selfprot" / "selfprot_kcat_input_metadata.csv"
DEFAULT_OUT_ROWS = BASE / "data" / "final" / "selfprot" / "selfprot_kcat_predictions_evaluated.csv"
DEFAULT_OUT_METRICS = BASE / "reports" / "tables" / "selfprot_eval_metrics.csv"
DEFAULT_OUT_MISSING = BASE / "data" / "final" / "selfprot" / "selfprot_invalid_or_unpredicted_rows.csv"
DEFAULT_OUT_MISSING_SUMMARY = BASE / "reports" / "tables" / "selfprot_invalid_or_unpredicted_summary.csv"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Score SELFprot output against benchmark truth.")
...
def detect_prediction(pred: pd.DataFrame, explicit: str = "") -> tuple[pd.Series, str]:
    ...
    raise ValueError(f"No recognized SELFprot prediction column found. Columns: {list(pred.columns)}")
...
def combine_predictions(pred: pd.DataFrame, meta: pd.DataFrame, pred_log10: pd.Series, pred_col: str) -> pd.DataFrame:
    ...
    optional = [
        ...
        for column in [
            "prediction_kcat",
            "selfprot_prediction_sd",
            "selfprot_folder",
            "selfprot_fold",
        ]
        if column in pred.columns
    ]
    if "selfprot_row_id" in pred.columns and "selfprot_row_id" in meta.columns:
        pred["selfprot_row_id"] = pd.to_numeric(pred["selfprot_row_id"], errors="coerce").astype("Int64")
        meta["selfprot_row_id"] = pd.to_numeric(meta["selfprot_row_id"], errors="coerce").astype("Int64")
        keep = ["selfprot_row_id", "prediction_log10", "prediction_column"] + optional
        combined = meta.merge(pred[keep], on="selfprot_row_id", how="inner", validate="one_to_one")
        ...
        raise ValueError("Need selfprot_row_id or entry_id in both predictions and metadata.")
...
def missing_rows(meta: pd.DataFrame, rows: pd.DataFrame) -> pd.DataFrame:
    if "selfprot_row_id" in meta.columns and "selfprot_row_id" in rows.columns:
        return meta.loc[~meta["selfprot_row_id"].isin(rows["selfprot_row_id"])].copy()
...
def missing_reason(row: pd.Series) -> str:
    if not bool(row.get("selfprot_smiles_valid", True)):
