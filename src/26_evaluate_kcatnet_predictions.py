#!/usr/bin/env python3
"""Evaluate KcatNet kcat predictions against the finalized benchmark truth."""
...
DEFAULT_PREDICTIONS = BASE / "data" / "final" / "kcatnet" / "kcatnet_kcat_input_output.csv"
DEFAULT_METADATA = BASE / "data" / "final" / "kcatnet" / "kcatnet_kcat_input_metadata.csv"
DEFAULT_OUT_ROWS = BASE / "data" / "final" / "kcatnet" / "kcatnet_kcat_predictions_evaluated.csv"
DEFAULT_OUT_METRICS = BASE / "reports" / "tables" / "kcatnet_eval_metrics.csv"
DEFAULT_OUT_MISSING = BASE / "data" / "final" / "kcatnet" / "kcatnet_invalid_or_unpredicted_rows.csv"
DEFAULT_OUT_MISSING_SUMMARY = BASE / "reports" / "tables" / "kcatnet_invalid_or_unpredicted_summary.csv"

LOG_COLUMNS = ["prediction_log10", "pred_log10_kcat", "predicted_log10_kcat", "log10_kcat"]
LINEAR_COLUMNS = ["Predicted Kcats", "prediction_kcat", "pred_kcat", "predicted_kcat", "kcat", "prediction"]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Score KcatNet output against benchmark truth.")
...
def detect_prediction(pred: pd.DataFrame, explicit: str = "") -> tuple[pd.Series, str]:
    ...
    raise ValueError(f"No recognized KcatNet prediction column found. Columns: {list(pred.columns)}")
...
def combine_predictions(pred: pd.DataFrame, meta: pd.DataFrame, pred_log10: pd.Series, pred_col: str) -> pd.DataFrame:
    ...
    optional = []
    rename_optional = {
        "Pro_seq": "kcatnet_output_sequence",
        "Smile": "kcatnet_output_smiles",
        "Predicted Kcats": "kcatnet_output_predicted_kcat",
    }
    ...
    if "kcatnet_row_id" in pred.columns and "kcatnet_row_id" in meta.columns:
        pred["kcatnet_row_id"] = pd.to_numeric(pred["kcatnet_row_id"], errors="coerce").astype("Int64")
        meta["kcatnet_row_id"] = pd.to_numeric(meta["kcatnet_row_id"], errors="coerce").astype("Int64")
        keep = ["kcatnet_row_id", "prediction_log10", "prediction_column"] + optional
        combined = meta.merge(pred[keep], on="kcatnet_row_id", how="inner", validate="one_to_one")
        ...
        if len(pred) != len(meta):
            raise ValueError(
                ...
                "and no kcatnet_row_id or entry_id column is available for merging."
            )
        combined = pd.concat(
            [
...
def missing_rows(meta: pd.DataFrame, rows: pd.DataFrame) -> pd.DataFrame:
    if "kcatnet_row_id" in meta.columns and "kcatnet_row_id" in rows.columns:
        return meta.loc[~meta["kcatnet_row_id"].isin(rows["kcatnet_row_id"])].copy()
...
def missing_reason(row: pd.Series) -> str:
    if not bool(row.get("kcatnet_smiles_valid", True)):
